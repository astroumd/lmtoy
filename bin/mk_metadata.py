...
if __name__ == "__main__":
    ...
    if input_yamlfile is None:
        ...
        if isCombined:
            ...
            obsinfo["subObsNum"] = int(
                header(rc, "subobsnum", debug)
            )  # normally 0 for us
        ...
        if (
            instrument == "SEQUOIA" or instrument == "MSIP1MM"
        ):  # @todo this is only for mapping so far
            ...
            inttime = float(
                header(rc, "inttime", debug)
            )  # oddity, SEQ has this in the bank based rc
            ...
            band["qaGrade"] = qagrade = get_qagrade(
                rc, debug
            )  # -2 .. 3 (-1,0 means not graded)

            band["nchan"] = nchan
            band[
                "bandName"
            ] = "OTHER"  # we don't have special names for the spectral line bands
